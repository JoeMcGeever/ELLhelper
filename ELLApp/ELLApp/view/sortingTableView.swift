...
class SortingViewController: UITableViewController {
    ...
    let arrayOfCategories = [["Unhealthy", "Fruit", "Vegetable"], ["Blue", "Red", "Green"], ["Fish", "Bird", "Insect"]]
    let arrayOfCatNames = ["Food", "Colour", "Animal"]
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath){
        ...
        performSegue(withIdentifier: "segueToSorting", sender: nil)
        
    }
    
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        //sens the account instance to the main menu page
        if segue.identifier == "segueToSorting" {
            let sortView = segue.destination as! sorting
            sortView.categories = selectedCategory //send the selected story to the title page
        }
